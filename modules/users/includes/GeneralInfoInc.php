...
if ($_REQUEST['profile'] != 'none') {
    ...
    if (!isset($staff['STAFF_ID'])) {
        //for adding new user
        echo TextInput(str_repeat('*', 8), 'login_authentication[PASSWORD]', '', "size=25 maxlength=100 class=cell_floating AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);validate_password(this.value);");
    } else {
        //for existing users while updating
        echo TextInput(str_repeat('*', 8), 'login_authentication[PASSWORD]', '', "size=25 maxlength=100 class=cell_floating AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);validate_password(this.value,$staff[STAFF_ID]);");
    }
