...
if ($_REQUEST['modfunc'] == 'delete' && (User('PROFILE') == 'admin' || User('PROFILE') == 'student')) {
    ...
    if (DeletePromptFilesEncoded($_REQUEST['title'], '&include=FilesInc3&category_id=' . $_REQUEST['category_id'])) {
...
if (!$_REQUEST['modfunc']) {
            ...
            if ($_FILES['uploadfile']['name'][$i]) {
                ...
                if ($_FILES["uploadfile"]["error"][$i] > 0) {
                    ...
                    else
                    {
                        ...
                        DBQuery('INSERT INTO user_file_upload (USER_ID,PROFILE_ID,SCHOOL_ID,SYEAR,NAME, SIZE, TYPE, CONTENT,FILE_INFO) VALUES (' . UserStudentID() . ',\'3\',' . UserSchool() . ',' . UserSyear() . ',\'' . addslashes($fileName) . '\', \'' . addslashes($fileSize) . '\', \'' . addslashes($fileType) . '\', \'' . addslashes($content) . '\',\'studocuments\')');
    ...
    if (!isset($_SESSION['grid_msg'])) {
        ...
        $file_info = DBGet(DBQuery('SELECT * FROM user_file_upload WHERE USER_ID=' . UserStudentID() . ' AND PROFILE_ID=3 AND SCHOOL_ID=' . UserSchool() . ' AND file_info=\'studocuments\''));
        echo '<tbody>';
        $found = false;
        $gridClass = "";
        $file_no = 1;
